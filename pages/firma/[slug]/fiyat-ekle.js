                ...
                {firmContent.servicePointsCombined.map((item, index) => {
                  ...
                  const cityApi = item.district
                    ? turkeyApi.provinces
                        ...
                        .districts.find((a) => a.id === parseInt(item.district))
                  ...
                  return (
                    ...
                    >
                      {({ setFieldValue, errors, touched, values, status }) => {
                        const handleInputChange = (e, setFieldValue) => {
                          const { name, value } = e.target
                          setFieldValue(name, value)
                          const minPrice = Number(
                            values.minimum_price.substring(1).replace(",", ".")
                          )
                          const maxPrice = Number(
                            values.maximum_price.substring(1).replace(",", ".")
                          )
                        ...
                        return (
                          <Form
                            className={`p-2 mb-6 lg:mb-2 border ${
                              ...
                              errors.api ? "border-danger" : "border-lightgray"
                            }`}
                          >
...
export const getServerSideProps = async (context) => {
  ...
  const { name, slug, servicePoints, firm_category, video, user } =
    firmData.attributes
